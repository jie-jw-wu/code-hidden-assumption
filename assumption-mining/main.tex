...
\sysname (heuristic)   & 0.478 & 0.480 & T6 (0.75) & T3 (0.44) \\
...
achieves 47.8\% accuracy (mean IoU\,=\,0.480) at the 0.5 threshold, within 5.4pp
of the keyword-heuristic baseline (KH: 53.2\%).  The accuracy gap relative to
the 20-task pilot reflects harder tasks in the expansion (class-based code with
multi-method interactions, e.g., Queue, MessageBroker), where keyword overlap
is lower across method boundaries.  The LLM-guided pass, which uses semantic
descriptions to identify regions rather than surface keywords, is expected to
achieve higher accuracy on the full \benchname by capturing assumptions with no
obvious identifier-level cues.  Full-File (FF) degrades sharply on larger
classes (IoU\,=\,0.188) as the ``entire file'' span grows.

\begin{rqbox}{RQ2 Summary (pilot, $n=30$)}
The \sysname heuristic dependency mapper achieves 47.8\% accuracy (mean
IoU\,=\,0.48) on the 30-task pilot, within 5.4pp of the keyword-heuristic
baseline (53.2\%).  The gap motivates the LLM-guided localization pass, which
uses semantic descriptions to find regions with no keyword cues.
\end{rqbox}
