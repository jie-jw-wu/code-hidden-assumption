...
\begin{table}[t]
\caption{\benchname benchmark statistics (pilot $n=10$; target $n=150$).
...
\midrule
Total tasks (target)                    & 150 (pilot: 10) \\
Tasks from HumanEval                    & 50 (pilot: 0) \\
Tasks from MBPP                         & 50 (pilot: 0) \\
Novel tasks                             & 50 (pilot: 10) \\
\midrule
Total ground-truth assumptions          & \todonumber{X} (pilot: 39) \\
Mean assumptions per task               & \todonumber{X} (pilot: 3.9) \\
Median assumptions per task             & \todonumber{X} (pilot: 4.0) \\
\midrule
T1 Input Validation                     & \todonumber{X} (pilot: 5, 12.8\%) \\
T2 Data Format                          & \todonumber{X} (pilot: 13, 33.3\%) \\
T3 Error Policy                         & \todonumber{X} (pilot: 8, 20.5\%) \\
T4 Persistence                          & \todonumber{X} (pilot: 4, 10.3\%) \\
T5 Performance                          & \todonumber{X} (pilot: 7, 17.9\%) \\
T6 Security                             & \todonumber{X} (pilot: 2, 5.1\%) \\
\midrule
Mean lines of code per task             & \todonumber{X} (pilot: 10.0) \\
...
\begin{table}[t]
\caption{RQ2: Dependency accuracy on \benchname (pilot, $n=10$ tasks, 39 assumptions).
         Accuracy = fraction with IoU $\geq$ 0.5.  IoU = mean Intersection over Union.
         — = metric not applicable.}
\label{tab:rq2}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Method} & \multicolumn{2}{c}{\textbf{Line-level}} & \multicolumn{2}{c}{\textbf{Category breakdown (Acc.)}} \\
                & Acc.  & IoU  & Best cat.  & Worst cat. \\
\midrule
Keyword Heuristic       & 0.513 & 0.547 & T4 (—)   & T1 (—) \\
Full-File Dependency    & 0.154 & 0.240 & —        & — \\
\sysname (heuristic)   & 0.333 & 0.346 & T4 (0.75) & T1 (0.00) \\
...
\textbf{Findings.}
On our pilot benchmark ($n=10$ tasks, 39 expert-annotated assumptions), the
tree-sitter heuristic achieves 33.3\% accuracy (mean IoU\,=\,0.35) at the
default 0.5 threshold.  The keyword-heuristic baseline (KH) achieves a higher
51.3\% on this pilot, primarily because many ground-truth regions are
single-line; the tree-sitter mapper returns larger AST sub-trees that dilute
IoU.  The LLM-guided pass (full \sysname, §\ref{sec:dependency}) is expected to
close this gap by using semantic descriptions rather than surface keywords;
results on the complete 150-task \benchname are pending.  T4 (Persistence) is
the easiest category to localise (0.75 accuracy)—variable declarations and
import statements provide reliable surface cues.  T1 (Input Validation) is
hardest (0.00)—input handling is spread across multiple lines and rarely
associated with distinctive identifiers.

\begin{rqbox}{RQ2 Summary (pilot)}
The heuristic dependency mapper localises 33\% of assumptions to the correct
code region (IoU\,$\geq$\,0.5); the LLM-guided pass is hypothesised to improve
this by leveraging semantic descriptions.  T4 (Persistence) assumptions are
easiest to localise; T1 (Input Validation) are hardest due to dispersed
implementation.
\end{rqbox}
