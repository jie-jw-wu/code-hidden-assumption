...
\begin{table}[t]
\caption{\benchname benchmark statistics (pilot $n=20$; target $n=150$).
...
\midrule
Total tasks (target)                    & 150 (pilot: 20) \\
Tasks from HumanEval                    & 50 (pilot: 0) \\
Tasks from MBPP                         & 50 (pilot: 0) \\
Novel tasks                             & 50 (pilot: 20) \\
\midrule
Total ground-truth assumptions          & \todonumber{X} (pilot: 74) \\
Mean assumptions per task               & \todonumber{X} (pilot: 3.7) \\
Median assumptions per task             & \todonumber{X} (pilot: 4.0) \\
\midrule
T1 Input Validation                     & \todonumber{X} (pilot: 12, 16.2\%) \\
T2 Data Format                          & \todonumber{X} (pilot: 21, 28.4\%) \\
T3 Error Policy                         & \todonumber{X} (pilot: 16, 21.6\%) \\
T4 Persistence                          & \todonumber{X} (pilot: 7, 9.5\%) \\
T5 Performance                          & \todonumber{X} (pilot: 14, 18.9\%) \\
T6 Security                             & \todonumber{X} (pilot: 4, 5.4\%) \\
\midrule
...
\begin{table}[t]
\caption{RQ1: Assumption extraction quality (pilot, $n=20$ tasks, 74 assumptions from
...
Pattern Extraction   & \textbf{0.068} & \textbf{0.278} & \textbf{0.109} & — & — \\
\midrule
...
\textbf{Findings (offline pilot, $n=20$).}
...
sentinels, exception breadth, hash-map use), achieves $P=0.28$, $R=0.07$,
$F_1=0.11$—better than CE but still low recall, because most assumptions require
language-level semantic reasoning rather than syntactic pattern matching.
T3 (Error Policy) and T2 (Data Format) are best covered by KBE since they
often manifest as explicit return-value patterns; T1 (Input Validation) and T6
(Security) are hardest for offline methods.
LLM-based baselines (DG, CQ, CoT) and \sysname results require API access and
will be reported in the full evaluation.

\begin{rqbox}{RQ1 Summary (pilot)}
Offline pattern-matching achieves only $F_1=0.11$ on the AmbigBench pilot,
confirming that implicit assumptions require LLM-level semantic understanding.
Comment-mining fails completely ($F_1=0.00$), as LLM-generated code rarely
documents unstated design decisions.
\end{rqbox}
...
\begin{table}[t]
\caption{RQ2: Dependency accuracy on \benchname (pilot, $n=20$ tasks, 74 assumptions).
...
\midrule
Keyword Heuristic (KH)  & 0.541 & 0.534 & T6 (—) & T3 (—) \\
Full-File (FF)          & 0.108 & 0.220 & — & — \\
\sysname (heuristic)   & 0.527 & 0.524 & T6 (0.75) & T3 (0.44) \\
...
\textbf{Findings.}
On our pilot benchmark ($n=20$ tasks, 74 expert-annotated assumptions), the
\sysname heuristic (tree-sitter + AST keyword-narrowing pass, §\ref{sec:dependency})
achieves 52.7\% accuracy (mean IoU\,=\,0.524) at the 0.5 threshold, matching
the keyword-heuristic baseline (KH: 54.1\%).  This parity is expected: the
narrowing pass reduces AST nodes to the keyword-matching lines, approaching KH
behaviour, while also handling multi-line algorithm blocks (T5) where KH
underperforms.  The LLM-guided pass, which uses semantic descriptions to
identify regions rather than surface keywords, is expected to achieve higher
accuracy on the full \benchname by capturing assumptions with no obvious
identifier-level cues.  T6 (Security) is the easiest category
(0.75 accuracy)---security assumptions are anchored to specific function calls
(\texttt{md5}, \texttt{secrets.choice}).  T3 (Error Policy) is hardest
(0.44)---error handling is dispersed across conditionals and raise/return
statements that share no distinctive identifier.

\begin{rqbox}{RQ2 Summary (pilot, $n=20$)}
The \sysname heuristic dependency mapper achieves 52.7\% accuracy (mean
IoU\,=\,0.52) on the pilot benchmark, matching the keyword-heuristic baseline
(54.1\%).  The LLM-guided pass is expected to exceed both by leveraging
semantic assumption descriptions.  T6 (Security) assumptions are easiest to
localise; T3 (Error Policy) are hardest.
\end{rqbox}
