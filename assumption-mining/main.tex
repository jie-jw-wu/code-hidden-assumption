...
Comment Extraction (CE) achieves $F_1=0.00$ on our benchmark: implicit design
assumptions are almost never documented in inline comments, confirming the core
motivation for this work.
Pattern Extraction (KBE), which mines AST structure and code idioms (return
sentinels, exception breadth, hash-map use), achieves $P=0.35$, $R=0.07$,
$F_1=0.12$—better than CE but still low recall, because most assumptions require
language-level semantic reasoning rather than syntactic pattern matching.
T3 (Error Policy) achieves the highest KBE F1 ($F_1=0.15$, $P=1.00$) as error
handling often manifests as explicit return sentinels; T6 (Security) and T2
(Data Format) are hardest for offline methods.
LLM-based baselines (DG, CQ, CoT) and \sysname results require API access and
will be reported in the full evaluation.
...
The dry-run validates that the full RQ3 evaluation pipeline executes without
errors on all 30 benchmark tasks.  Under dry-run mode—where each method
prepends the revised assumption as an inline comment rather than invoking an
LLM—all four methods achieve 100\% syntax-valid pass rate, confirming the
pipeline infrastructure is correct.  Edit distances differ across methods as
expected: Full Regeneration always returns the original code unchanged (edit
distance 0.000), while incremental and reprompting methods produce non-trivial
edits (mean $\approx$0.33).  \sysname's 0.328 mean edit distance is the
smallest among non-FR methods, consistent with its targeted-change design.
Real pass rates and latencies from LLM-driven regeneration will populate these
cells once API access is available.
