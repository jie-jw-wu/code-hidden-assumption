...
\begin{enumerate}[noitemsep]
  \item \textbf{LLM pass.}  The extraction prompt (\cref{lst:prompt}) requests
        ...
        with a token-overlap heuristic: we tokenize both the LLM's region
        description and each code identifier, then retain AST nodes whose
        identifier tokens share a Jaccard similarity $\geq 0.25$ with the
        description tokens (threshold chosen by pilot on 20 held-out tasks).
...
The expanded node-type coverage—adding \texttt{with\_statement} and \texttt{call}
to T3, and \texttt{import\_from\_statement} to T5—allows the mapper to find
error-handling assumptions anchored to \texttt{with open()} calls and
performance assumptions anchored to library import lines.
